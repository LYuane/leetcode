...
class Solution {
    public int rob(int[] nums) {
        ...
        return Math.max(rob_no_circle(Arrays.copyOfRange(nums,0,nums.length-1)),rob_no_circle(Arrays.copyOfRange(nums,1,nums.length)));
    }

    public int rob_no_circle(int[] nums) {
        if(nums.length < 1){
            return 0;
        }
